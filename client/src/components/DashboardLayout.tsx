  ...
  return (
    <div className="grid min-h-screen place-items-center bg-[radial-gradient(circle_at_top_left,#ecfdf5,transparent_38%),#f8fafc] p-6">
      <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-[0_30px_80px_-40px_rgba(15,23,42,.3)]">
        <img src={ISEYC_LOGO} alt="Official ISEYC logo" className="mx-auto h-16 w-16 rounded-2xl object-cover shadow-sm" />
        ...
        {children}
        <p className="mt-5 text-xs text-slate-400">Empowering Youths, Shaping Communities.</p>
      </div>
    </div>
  );
}
  ...
  return (
    <>
      <div ref={sidebarRef} className="relative">
        <Sidebar collapsible="icon" className="!border-r !border-slate-800 !bg-slate-950 text-slate-200" disableTransition={isResizing}>
          <SidebarHeader className="h-24 justify-center !border-b !border-slate-800 !bg-slate-950">
            <div className="flex items-center gap-3 px-3">
              <button onClick={toggleSidebar} className="grid h-8 w-8 place-items-center rounded-lg text-slate-400 transition-colors hover:bg-slate-800 hover:text-white">
                <PanelLeft className="h-4 w-4" />
              </button>
              {!isCollapsed ? (
                <div className="flex min-w-0 items-center gap-2">
                  <img src={ISEYC_LOGO} alt="ISEYC" className="h-10 w-10 rounded-xl object-cover" />
          ...
          <SidebarContent className="!bg-slate-950 pt-5">
            {["Executive", "Development", "Operating records", "Communications", "Governance"].map(group => {
              ...
              return (
                <div key={group} className="mb-5">
                  <p className="mb-2 px-5 text-[10px] font-bold uppercase tracking-[.16em] text-slate-500 group-data-[collapsible=icon]:hidden">{group}</p>
            ...
            <div className="mx-4 mt-5 border-t border-slate-800 pt-5 group-data-[collapsible=icon]:hidden">
              <div className="flex gap-2 text-xs leading-5 text-slate-400">
                ...
                Human authority remains final for records, actions, decisions, development guidance, and external communication.
              </div>
            </div>
          </SidebarContent>
          <SidebarFooter className="!border-t !border-slate-800 !bg-slate-950 p-3">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex w-full items-center gap-3 rounded-lg px-1 py-1 text-left hover:bg-slate-800">
                  <Avatar className="h-9 w-9 border border-slate-700">
                    <AvatarFallback className="bg-slate-800 text-xs text-emerald-200">{user?.name?.charAt(0).toUpperCase()}</AvatarFallback>
                  </Avatar>
                  <div className="min-w-0 flex-1 group-data-[collapsible=icon]:hidden">
                    <p className="truncate text-sm font-medium text-white">{user?.name || "ISEYC Member"}</p>
                    <p className="mt-1 truncate text-[10px] font-bold uppercase tracking-[.11em] text-emerald-300">{docRoleLabel}</p>
                  </div>
                </button>
              </DropdownMenuTrigger>
        ...
        <div
          className={`absolute right-0 top-0 h-full w-1 cursor-col-resize hover:bg-emerald-400/40 ${isCollapsed ? "hidden" : ""}`}
          onMouseDown={() => setIsResizing(true)}
        />
      </div>
      <SidebarInset className="bg-[radial-gradient(circle_at_top_right,#ecfdf5,transparent_26%),#f8fafc]">
        {isMobile ? (
          <header className="sticky top-0 z-40 flex h-14 items-center gap-3 border-b border-slate-200 bg-white/90 px-3 backdrop-blur">
            <SidebarTrigger />
            <span className="font-serif text-lg text-slate-900">{active}</span>
          </header>
        ) : null}
